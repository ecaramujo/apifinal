...
exports.devolver = (req, res) => {
    ...
    con.query(sql, ["Devolvido", "2021-06-18T03:00:00.000Z", id_locacao], (err, rows) => {
        if (err) {
            res.status(500).json({"err:":"Database err"})
            console.log(err)
        }
            ...
            const date2 = new Date("2021-06-20T03:00:00.000Z");
            ...
            if (date1 < date2){
                res.json({"msg:":`O livro foi devolvido ! Ele está com um atraso de ${diferenca} dias !`})
            }
            else{
